...
5. **Phase 4 — Policy, Risk & governance evidence (in progress):** first explicit Agent-to-Model permission and persisted decision slice implemented early; successful permission PUTs now append atomic, fingerprint-only Management audit evidence with a bounded Tenant read API. Static budget edits, failure-attempt evidence, dynamic grants, richer conditions, protected retention, and export remain planned.
