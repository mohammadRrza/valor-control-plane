...
4. **Phase 3 — Usage and observability (in progress):** durable Invocation duration, normalized provider usage, safe provider response correlation, and bounded Tenant-scoped usage/estimated-cost reporting are implemented. Metrics export, tracing backends, dashboards, alerts, and retention/redaction remain planned; no OpenTelemetry stack is present.
