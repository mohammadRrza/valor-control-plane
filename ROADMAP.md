...
4. **Phase 3 — Usage and observability (in progress):** durable Invocation duration, normalized provider usage, and safe provider response correlation are implemented. Metrics aggregation/export, tracing backends, dashboards, alerts, and retention/redaction remain planned; no OpenTelemetry stack is present.
...
8. **Phase 7 — AI FinOps:** usage-fact persistence and a sequential UTC-daily Runtime Principal total-unit pre-check are implemented early; pricing, monetary cost calculation, tenant budgets, strict concurrent quotas, and broader cost controls remain planned.
